  ...
  return {
    ...
    cacheHitRate:
      stats.total_requests > 0
        ? (stats.cache_hits || 0) / stats.total_requests
        : 0,
    avgLatencyMs: stats.avg_latency || 0,
    modelUsage: Object.fromEntries(modelUsage.map((r) => [r.model, r.count])),
    taskTypeUsage: Object.fromEntries(
      taskUsage.map((r) => [r.task_type, r.count]),
    ),
  };
}
...
export function getTotalSpend(userId: string, since: number): number {
  const result = getDb()
    .prepare(
      "SELECT SUM(cost) as total FROM requests WHERE user_id = ? AND timestamp >= ?",
    )
...
export function getTotalSavings(userId: string, since: number): number {
  const result = getDb()
    .prepare(
      "SELECT SUM(cost_saved) as total FROM requests WHERE user_id = ? AND timestamp >= ?",
    )
    .get(userId, since) as { total: number | null };
  return result?.total || 0;
}
