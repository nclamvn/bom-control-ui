...
export function checkBudget(
  ...
  const dailySpend = metrics.getTotalSpend(
    userId,
    now - 24 * 60 * 60 * 1000,
  );
  ...
  const weeklySpend = metrics.getTotalSpend(
    userId,
    now - 7 * 24 * 60 * 60 * 1000,
  );
  ...
  const monthlySpend = metrics.getTotalSpend(
    userId,
...
export function generateReport(
  ...
  if (expensiveUsage > stats.totalRequests * 0.3) {
    recommendations.push(
      "Consider using cheaper models (Haiku, GPT-4o-mini) for simple tasks.",
    );
  }
  ...
  return {
    ...
    savingsBreakdown: {
      ...
      percentOfSpend:
        stats.totalCost > 0
          ? (totalSaved / (stats.totalCost + totalSaved)) * 100
          : 0,
    },
  };
}
