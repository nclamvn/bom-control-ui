...
export function classifyTask(
  ...
  if (inputLength > 5000) {
    ...
    const isComplex = COMPLEXITY_COMPLEX_PATTERNS.some((p) =>
      p.test(fullText),
    );
  ...
  switch (detectedType) {
    ...
    case "coding":
      estimatedOutputTokens =
        complexity === "simple" ? 300 : complexity === "complex" ? 2000 : 800;
      break;
    default:
      estimatedOutputTokens =
        complexity === "simple" ? 200 : complexity === "complex" ? 2000 : 800;
  }
  ...
  return {
    ...
    estimatedOutputTokens,
    requiresReasoning:
      ["analysis", "coding"].includes(detectedType) && complexity === "complex",
    requiresCreativity: detectedType === "writing",
    requiresAccuracy: ["extraction", "translation", "coding"].includes(
      detectedType,
    ),
...
export function quickClassify(
  prompt: string,
): { type: TaskType; complexity: "simple" | "medium" | "complex" } {
