  ...
  for (const line of lines) {
    ...
    if (/^#+\s/.test(trimmed) || /^[A-Z][^a-z]*:/.test(trimmed)) {
      if (contentBuffer.length > 0) {
        ...
        result.push(
          estimateTokens(content) > 200
            ? extractKeySentences(content, targetRatio)
            : content,
        );
        contentBuffer = [];
      }
...
export function compressContext(
  ...
  const targetTokens =
    options.targetTokens || Math.ceil(originalTokens * targetRatio);
  ...
  if (compressedTokens > targetTokens * 1.5) {
    ...
    compressed =
      compressed.substring(0, targetChars) +
      "\n\n[Content truncated for length...]";
    compressedTokens = estimateTokens(compressed);
    method = "truncate";
  }

  return { compressed, originalTokens, compressedTokens, ratio: compressedTokens / originalTokens, method };
}
