  ...
  database
    ...
    .run(
      ...
      entry.promptEmbedding
        ? Buffer.from(new Float32Array(entry.promptEmbedding).buffer)
        : null,
      entry.response,
      entry.model,
      entry.tokens.input,
...
export function getByHash(hash: string): CacheEntry | null {
  ...
  const row = database
    .prepare(
      "SELECT * FROM cache_entries WHERE prompt_hash = ? AND expires_at > ?",
    )
  ...
  return rows
    ...
    .map((row) => ({
      ...
      embedding: Array.from(
        new Float32Array((row.prompt_embedding as Buffer).buffer),
      ),
    }));
}
...
export function getStats(): CacheStats {
  ...
  const statsRow = database
    .prepare("SELECT * FROM cache_stats WHERE id = 1")
